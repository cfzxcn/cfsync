...
先看网络环境的配置：在虚拟网络编辑器中，可以使用NAT模式或仅主机模式，它们的区别是：使用NAT模式的虚拟机可以访问互联网，使用仅主机模式的虚拟机不能访问互联网。这里以NAT模式为例，注意：要取消“使用本地的DHCP服务将ip地址分配给虚拟机”，因为我们要用一台配置好的服务端的dhcp服务来给客户端分配ip，而不是用vm的dhcp服务来分配ip
